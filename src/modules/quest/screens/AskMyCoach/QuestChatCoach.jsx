...
const ChatCoach = ({ navigation }) => {
  ...
  const filteredSimulationPrompts = simulationPrompts
    ...
    .flatMap((item) => item.prompts)
    .filter((item) =>
      item.title.toLowerCase().includes(searchQuerySimulation.toLowerCase())
    );
    ...
    >
      ...
      >
        <Image
          ...
          style={{
            alignSelf: "center", height: 200, resizeMode: "contain" 
          }}
        />
        <View style={{ width: "80%", marginVertical: "5%" }}>
          ...
          >
            Personalized Recruitment Coaching
          </Text>
        </View>
        ...
        <View
          style={{ width: "90%", alignItems: "center", marginBottom: "10%" }}
        >
          <View
            style={{
              flexDirection: "row",
              ...
              {filteredPrompts.length != 0 && (
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <Pressable
                      onPress={() =>
                        goToChatScreen(item.description, "Ready Prompt")
                      }
                      key={item.step}
                      style={{
      ...
      >
        <View style={[styles.searchContainer2]}>
          ...
          <TouchableOpacity onPress={() => setSearchQuerySimulation("")}>
            {chatQuery && (
              <Pressable
                onPress={() => sendCustomMessage(chatQuery)}
                style={{
                  borderRadius: 50,
                  padding: 5,
        ...
        >
          QUEST AI coach can make mistakes. Check important info.
        </Text>
      </View>
    </KeyboardAvoidingView>
  );
