...
import { setWeeklyAchieved, setYearlyAchieved } from "../../../redux/features/entriesSlice";
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: "row", alignItems: "center" }}>
        <Image
          source={require("../../../../assets/quest/goal1.png")}
          style={{
            marginRight: 2,
            width: 45,
            height: 45,
      ...
      <View style={{ flexDirection: "row", alignItems: "center" }}>
        <Image
          source={require("../../../../assets/quest/achieved1.png")}
          style={{
            marginRight: 2,
            width: 40,
            height: 40,
...
const EffectivenessReport = () => {
  ...
  const calculateContractsRatioGoal = (currentEntries) => {
    const contractsSigned = currentEntries?.SuccessFormula?.contractsSigned || 0;
    const presentationsHeld =
      currentEntries?.SuccessFormula?.presentationsHeld || 0;

    return (contractsSigned / presentationsHeld) * 100;
  };

  const calculateContractsRatioAchieved = (currentEntries) => {
    const yearlyAchievedPR = currentEntries?.yearly_achieved?.pr_yearly || 0;
    const yearlyAchievedC = currentEntries?.yearly_achieved?.c_yearly || 0;

    return (yearlyAchievedC / yearlyAchievedPR) * 100;
  };
  ...
  const calculateProspectingRatioGoal = (currentEntries) => {
    const yearlyGoal = currentEntries?.yearly_goals?.p_yearly || 0;
    return yearlyGoal ? 100 : 0;
  };

  const calculateProspectingRatioAchieved = (currentEntries) => {
    const yearlyGoal = currentEntries?.yearly_goals?.p_yearly || 0;
    const yearlyAchieved = currentEntries?.yearly_achieved?.p_yearly || 0;
    return yearlyGoal ? (yearlyAchieved / yearlyGoal) * 100 : 0;
  };
    ...
    <SafeAreaView style={styles.backgroundStyle}>
      ...
      >
        ...
        <View style={{ flex: 1, justifyContent: "flex-start" }}>
          ...
          <Category
            goal={calculateProspectingRatioGoal(entries)}
            achieved={calculateProspectingRatioAchieved(entries)}
            text="P"
          ...
          <Category
            goal={calculateContractsRatioGoal(entries)}
            achieved={calculateContractsRatioAchieved(entries)}
            text="C"
